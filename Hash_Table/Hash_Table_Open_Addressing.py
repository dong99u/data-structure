class HashOpenAddr:
    ...
    def find_slot(self, key):
        i = self.hash_function(key) # hash
        start = i
        while self.keys[i] != None and self.keys[i] != key:
            # When H[i] is unoccupied or H[i].key == key, break
            i = (i + 1) % self.size
            if i == start: # table is FULL
                return None
        return i
    ...
    def remove(self, key):
        ...
        if self.keys[i] == None:
            return None
        j = i
        while True:     # H[i] : empty slot, H[j] : 이사해야 할 slot
            self.keys[i] = None
            while True:
                j = (j + 1) % self.size
                if self.keys[j] == None:
                ...
                if not (i < k <= j or j < i < k or k <= j < i):
                    break
            self.keys[i] = self.keys[j] # 테이블 이사
            i = j
